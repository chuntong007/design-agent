...
import { Newspaper, MapPin, Anchor, X, ExternalLink, TrendingUp, TrendingDown, Minus, Globe2, Search, Loader2, AlertCircle, RefreshCw } from 'lucide-react'
import type { NewsItem, Fund } from '../types'
...
export const NewsPanel: React.FC<Props> = ({ triggerFund, triggerDate, triggerNav, anchorInfo, onAnchor, onClose }) => {
  ...
  return (
    <div className="h-full flex flex-col animate-slide-up">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {triggerFund && triggerDate
                ? `已检索「${triggerFund.name}」${triggerDate} 附近 ±3 日全球资讯`
                : '点击左侧净值曲线任意点位以检索新闻'}
            </p>
          </div>
        </div>
      ...
      {/* 锚定对照条 */}
      {anchorInfo && (
        <div className="px-5 py-2.5 bg-amber-50/70 border-b border-amber-100 flex items-center justify-between gap-3">
              ...
              {news.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed mb-2.5">{item.summary}</p>
