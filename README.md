...
在仓库根目录执行（跨平台，Win / macOS / Linux 通用，首次需先安装依赖）：

```bash
npm run install:all     # 首次：安装 根 / server / client 全部依赖
npm run dev             # 一键启动前后端，Ctrl+C 同时停止
```

- 前端 http://localhost:5173 ，后端 http://localhost:8787
- 基于 `concurrently` 并行启动，输出带 `[server]` / `[client]` 前缀，日志一目了然
